...
from controller.Funciones import eliminar_mesero
...
class EliminarMesero():
    def __init__(self):
        ...
        self.ventanaCliente.title("Eliminar Mesero")
        
        #Titulo principal
        self.lblTituloPrincipal = Label(self.ventanaCliente, text="Eliminar Mesero", font=("Arial", 15))
        ...
        #Boton Buscar
        self.btnBuscar = Button(self.ventanaCliente, image = img_buscar, command=self.buscar_mesero)
        self.btnBuscar.place(x=60, y=370, height=40, width=110)
        self.btnBuscar.image = img_buscar
        self.create_tooltip(self.btnBuscar, "Buscar Mesero")
        
        #Boton Guardar
        self.btnGuardar = Button(self.ventanaCliente, image = img_guardar, command=self.eliminar_mesero) 
        self.btnGuardar.place(x=190, y=370, height=40, width=110)
        self.btnGuardar.image = img_guardar
        self.create_tooltip(self.btnGuardar, "Eliminar Mesero")
    ...
    def eliminar_mesero(self):
        ...
        try:
            # Llama a la función eliminar_mesero con los datos
            eliminar_mesero(cedula, nombre, apellido, telefono, email)
            ...
            messagebox.showerror("Error", f"Ocurrió un error al buscar/eliminar el mesero: {e}")

    #funcion para buscar el mesero
    def buscar_mesero(self):
        """Busca un mesero por cédula y llena los campos deshabilitados si existe."""
        cedula = self.txtCedula.get()
        ...
        try:
            ...
            # Consulta para buscar un mesero por cédula
            consulta = "SELECT nombre, apellido, telefono, email FROM mesero WHERE cedula = %s"
            ...
            if resultado:
                ...
                messagebox.showwarning("Advertencia", "No se encontró un mesero con la cédula proporcionada")

        except Exception as e:
            messagebox.showerror("Error", f"Ocurrió un error al buscar el mesero: {e}")
        finally:
