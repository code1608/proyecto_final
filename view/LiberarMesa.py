...
class LiberarMesa():
    def __init__(self):
        ...
        self.ventanaCliente.title("Liberar Mesa")
        
        #Titulo principal
        self.lblTituloPrincipal = Label(self.ventanaCliente, text="Liberar Mesa", font=("Arial", 15))
        ...
        #Boton Guardar
        self.btnGuardar = Button(self.ventanaCliente, image = img_guardar, command=self.liberar_mesa)
        self.btnGuardar.place(x=210, y=270, height=40, width=110)
        self.btnGuardar.image = img_guardar
        self.create_tooltip(self.btnGuardar, "Liberar Mesa")
    ...
    def liberar_mesa(self):
        """Actualiza el estado de una mesa a 'libre' si está ocupado."""
        id = self.txtId.get()
        estado = self.txtEstado.get()

        if not id or not estado:
            messagebox.showwarning("Advertencia", "Debe buscar una mesa antes de liberarla")
            return

        if estado.lower() != "ocupado":
            messagebox.showwarning("Advertencia", "La mesa no está ocupada para ser liberada")
            return
        ...
        try:
            ...
            consulta = "UPDATE mesa SET estado = 'libre' WHERE id = %s"
            cursor.execute(consulta, (id,))
            con.commit()

            messagebox.showinfo("Éxito", "La mesa ha sido liberada correctamente")

            # Actualizar el campo de estado
            self.txtEstado.config(state="normal")
            self.txtEstado.delete(0, 'end')
            self.txtEstado.insert(0, "libre")
            self.txtEstado.config(state="readonly")

        except Exception as e:
            messagebox.showerror("Error", f"Ocurrió un error al liberar la mesa: {e}")
        finally:
            mi_conexion.cerrarConexion()
