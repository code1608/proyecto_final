...
from controller.Funciones import registrar_mesero


class RegistrarMesero():
    def __init__(self):
        self.ventanaCliente = tk.Toplevel()
        self.ventanaCliente.config(width="500", height="450")
        self.ventanaCliente.resizable(0,0)
        self.ventanaCliente.title("Registrar Mesero")
        
        #Titulo principal
        self.lblTituloPrincipal = Label(self.ventanaCliente, text="Registrar Mesero", font=("Arial", 15))
        ...
        #Boton Guardar
        self.btnGuardar = Button(self.ventanaCliente, image = img_guardar, command=self.guardar_mesero)
        self.btnGuardar.place(x=140, y=370, height=40, width=110)
        self.btnGuardar.image = img_guardar
        self.create_tooltip(self.btnGuardar, "Guardar Mesero")
    ...
    def guardar_mesero(self):
        ...
        try:
            # Llama a la función de registrar_mesero
            registrar_mesero(cedula, nombre, apellido, telefono, email)
